...
import {
  Users, AlertCircle, RefreshCw, Download,
  CheckCircle, Clock, MessageCircle, Phone, Calendar,
} from "lucide-react";
import { getLeads, getWhatsappLeads } from "@/lib/leadsApi";
...
export default function Dashboard() {
  ...
  // Total follow-ups
  const totalFollowUps = leads.filter((l) => l.followUp && l.followUp.trim() !== "");

  // Not contacted — leads with no followUp date set
  const notContactedLeads = leads.filter((l) => !l.followUp || l.followUp.trim() === "");
  ...
  const pipelineRows: {
    ...
    { label: "New", count: leads.filter((l) => l.status === "new").length, colorIndex: 0 },
    { label: "Contacted", count: leads.filter((l) => l.status === "contacted").length, colorIndex: 1 },
    // ── NEW ROW ──
    { label: "Not Contacted", count: notContactedLeads.length, colorIndex: 2, clickable: true, filterKey: "no-followup" },
    { label: "Overdue Follow-ups", count: overdueLeads.length, colorIndex: 5 },
    { label: "Converted", count: convertedLeads.length, colorIndex: 4 },
    { label: "Lost", count: lostLeads.length, colorIndex: 5 },
  ];
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card
  className="cursor-pointer hover:border-orange-400 transition"
  onClick={() => navigate("/leads?filter=no-followup")}
>
  <CardHeader className="flex flex-row items-center justify-between pb-2">
  ...
  <CardContent>
    <div className="text-2xl font-bold text-orange-600">{notContactedLeads.length}</div>
    <p className="text-xs text-muted-foreground">no follow-up scheduled</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{totalFollowUps.length}</div>
            <p className="text-xs text-muted-foreground">{todayFollowUps.length} scheduled today</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {pipelineRows.map(({ label, count, colorIndex, clickable, filterKey }) => {
              const pct = totalLeads > 0 ? Math.round((count / totalLeads) * 100) : 0;
              return (
                <div key={label}>
