...
export type Lead = {
  ...
  assignedTo: string;
  followUp: string;
  tags?: string[] | string;  // may be array or string after MySQL
...
// Robust tags array normalization for every lead
function addDuplicateInfo(leadsData: any): Lead[] {
  if (!Array.isArray(leadsData)) {
    console.error("Expected array for leadsData but got:", leadsData);
    return [];
  }
  return leadsData.map((lead) => {
    ...
    const duplicateMatches = leadsData.filter((l) => {
      ...
      const sameName =
        validName &&
        (l.name || "").toLowerCase().trim() === (lead.name || "").toLowerCase().trim();
      const sameContact =
        validContact &&
        (l.contact || "").toLowerCase().trim() === (lead.contact || "").toLowerCase().trim();
...
export default function Leads() {
  ...
  const [mergeModal, setMergeModal] = useState<{ primary: Lead | null; duplicates: Lead[] }>({
    primary: null,
    duplicates: [],
  });
  ...
  async function fetchLeads() {
    ...
    if (assignedTo !== "all" && user?.role === "admin") params.assignedTo = assignedTo;
    ...
    try {
      ...
      if (Array.isArray(res.data)) {
        ...
        console.error("Backend /api/leads did not return an array:", res.data);
      ...
      try {
        ...
        console.error("WhatsApp leads fetch failed", waErr);
      }

      setLeads(addDuplicateInfo([...regularLeads, ...whatsappLeadsNormalized]));
      setError(null);
    } catch (err) {
  ...
  function handleDeleteLead(id: number) {
    if (window.confirm("Are you sure you want to delete this lead?")) {
      ...
      deleteLead(id)
        .then(() => {
          setLeads((prev) => addDuplicateInfo(prev.filter((l) => l.id !== id)));
          setError(null);
        })
  ...
  function openMergeModal(lead: Lead) {
    ...
    if (duplicates.length === 0) {
      setError("No duplicates found for this lead");
      return;
    }
  ...
  function handleMergeComplete() {
    fetchLeads();
    setMergeModal({ primary: null, duplicates: [] });
    setError(null);
  }

  function handleCloseLead(lead: Lead) {
    handleUpdateLead({ ...lead, status: "closed" });
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            ...
            {showDuplicateFilter && <span className="text-sm text-yellow-600 ml-2">(Duplicates Only)</span>}
            {showWhatsappOnly && <span className="ml-2 text-blue-600 text-sm">[WhatsApp Only]</span>}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 px-4 font-medium">Duplicates</th>
                ...
                <tbody>
                  {filteredLeads.map((lead) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 text-xs text-muted-foreground">{lead.email || "-"}</td>
                      <td className="py-4 px-4">
                        <Badge
                          variant={
                            lead.source === "Meta Ad"
                      ...
                      <td className="py-4 px-4 text-sm">
                        ...
                        {isOverdue(lead.followUp) && (
                          <AlertCircle className="inline ml-1 text-red-600" size={18} title="Follow-up overdue" />
                        )}
                      </td>
                      ...
                      <td className="py-4 px-4 text-xs text-blue-700">
                      ...
                      <td className="py-4 px-4">
                        {lead.tags && Array.isArray(lead.tags) && lead.tags.length > 0 ? (
                          lead.tags.map((tag) => (
                            <span key={tag} className="inline-block bg-blue-100 text-blue-700 rounded px-2 mr-1 text-xs">
                              {tag}
                            </span>
                          ))
                        ) : (
                      ...
                      <td className="py-4 px-4">
                        <div className="flex gap-1 flex-wrap items-center">
                          <Button size="sm" variant="ghost" onClick={() => openEditModal(lead)} disabled={loading} title="Edit"><CheckCircle className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => handleDeleteLead(lead.id as number)} disabled={loading} title="Delete"><XCircle className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => setWAModalLead(lead)} disabled={loading} title="Send WhatsApp"><MessageCircle className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => copyToClipboard(lead.contact)} disabled={loading} title="Copy Contact"><Copy className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => setAssignLead(lead)} disabled={loading} title="Assign"><Users className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => setFollowUpLead(lead)} disabled={loading} title="Follow-Up"><Calendar className="h-4 w-4" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => handleCloseLead(lead)} disabled={loading} title="Close"><CheckCircle className="h-4 w-4" /></Button>
                          {lead.duplicateInfo?.isDuplicate && (
                            <Button size="sm" variant="outline" onClick={() => openMergeModal(lead)} disabled={loading} title="Merge Duplicates">Merge</Button>
                          )}
                        </div>
                      </td>
                    </tr>
      ...
      {waModalLead && (
        <WhatsAppTemplateModal lead={waModalLead} onClose={() => setWAModalLead(null)} />
      )}
      {duplicateAlertLead && (
        <DuplicateAlert lead={duplicateAlertLead} onClose={() => setDuplicateAlertLead(null)} />
      )}
